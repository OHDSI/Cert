...
SELECT DISTINCT A.PERSON_ID, A.VISIT_START_DATE, A.VISIT_END_DATE
	, MIN(B.DRUG_EXPOSURE_START_DATE) FIRST_DRUG_ORDDATE, B.DRUG_NAME
INTO VISIT_EXPOSURE
FROM (
...
FROM VISIT_EXPOSURE A
	INNER JOIN (
		SELECT AA.PERSON_ID, AA.OBSERVATION_CONCEPT_ID, CAST(AA.OBSERVATION_DATE AS DATETIME)+CAST(AA.OBSERVATION_TIME AS DATETIME) OBSERVATION_DATETIME, AA.VALUE_AS_NUMBER RESULT, AA.RANGE_LOW, AA.RANGE_HIGH
			, BB.LAB_NAME, BB.ABNORM_TYPE
		FROM (
				SELECT PERSON_ID, OBSERVATION_CONCEPT_ID, OBSERVATION_DATE, OBSERVATION_TIME, VALUE_AS_NUMBER, RANGE_LOW, RANGE_HIGH
...
FROM LAB_EXPOSURE A
	INNER JOIN (
		SELECT *
		FROM (
			SELECT PERSON_ID, FIRST_DRUG_ORDDATE, DRUG_NAME, OBSERVATION_CONCEPT_ID, RANGE_LOW, RANGE_HIGH, IS_BEFORE
			FROM LAB_EXPOSURE
		) A
		PIVOT (
...
IF OBJECT_ID('[@target_database].[dbo].SUMMARY', 'U') IS NOT NULL
	DROP TABLE SUMMARY;
SELECT PERSON_ID, DRUG_NAME, LAB_NAME, OBSERVATION_CONCEPT_ID, ABNORM_TYPE, RANGE_LOW, RANGE_HIGH, RESULT_BEFORE=Y, RESULT_AFTER=N, RESULT_TYPE
